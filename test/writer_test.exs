defmodule ExsodaTest.Writer do
  ...
  test "can write with a timeout set" do
    options =
      Writer.write(recv_timeout: 8000, timeout: 2000)
      |> Writer.create("a name", %{description: "describes"})
      |> Map.get(:opts)
  ...
  test "can write with parameters" do
    options =
      Writer.write(params: %{"k1" => "v1", "k2" => "v2"}, recv_timeout: 8000, timeout: 2000)
      |> Writer.create("a name", %{description: "describes"})
      |> Map.get(:opts)
  ...
  test "can create Upsert operation" do
    ...
    assert Enum.into(rows_stream, []) == [
             [{"text_column", "a text value"}],
             [{"text_column", "a second text value"}]
           ]
  end
  ...
  test "can add options to Upsert operation" do
    ...
    assert String.contains?(
             Response.get_private(results, :exsoda_request_url),
             "this_is_an_option=true"
           )
  end
  ...
  test "running Publish succeeds" do
    ...
    assert [{:ok, %Response{body: %{"id" => ^id, "publicationStage" => "published"}}}] = results
  end
  ...
  test "setting Site Permission fails for ODP domain" do
    ...
    assert {:ok, %{"message" => "Site scope not enabled", "error" => true}} =
  ...
  test "setting the Permissions blob succeeds" do
    ...
    assert {:ok, %{body: %{"pendingGrants" => [%{"flags" => ["public"]}]}}} =
             Reader.query(id)
             |> Reader.get_view()
  end
  ...
  test "can spoof a user during a write request" do
    ...
    opts = [
      {:spoof, spoof},
      {:host, "lb-vip.aws-us-west-2-staging.socrata.net:8081"},
      {:api_root, ""},
      {:protocol, "http"}
    ]
